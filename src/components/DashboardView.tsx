...
/**
 * The full dashboard, rendered for a given systemId. Used by the
 * owner's /dashboard (readOnly=false) and the public /share/[token]
 * read-only view (readOnly=true). The dashboard has no mutation
 * controls — every panel is display or navigation — so read-only only
 * hides the date-picker + PNG-export header controls; timeframe tabs
 * and the period calendar remain fully navigable.
 */
  ...
  return (
    <div className="p-3 md:p-6 space-y-4 md:space-y-6" id="dashboard-root">
      ...
      <header className="relative">
        ...
        {/* Controls row — date picker + PNG export (owner only). The
            pending-scale pill stays visible read-only since it's
            informational. */}
